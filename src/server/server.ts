...
export default class Server { //para exportar esta clase a otros archivos, default es para decir que Server se va exportar por defecto 
    /*creariamos una clase para manejar ese servidor, la idea es reutilizar esta codigo en otras aplicaciones*/
    public app: express.Application;
    public port: number; //definimos propiedades y el tipo de dato que va contener es este dato,
    ...
    */
    ...
    constructor( puerto:number ) { //cada que se crea una nueva instancia del server, voy a requerir el puerto
        this.port =puerto;
    ...
    private publicFolder () { //metodo privado, por que no quiero que sea accesado fuera de esta clase, va inicizalizar esta configuracion const publicPath = path.resolve(__dirname, '../public');
    ...
    start (callback: Function ) { //aqui ya escuchando y ponemos el callback
